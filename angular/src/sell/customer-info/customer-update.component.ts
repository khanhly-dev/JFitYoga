...
export class CustomerUpdateComponent implements OnInit {
  ...
  checkDuplicateCustomerUser()
  {
    return (cusUser : AbstractControl) => {
      return (this.customerList.filter(x => x.userName == cusUser.value).length > 0 && this.createOrUpdateForm.value.userName != this.customerBinding.userName) ? {
        invalidCusUser : true
      } : null
    };
  }
  ...
  checkDuplicatePhoneNumber()
  {
    return (phoneNumber : AbstractControl) => {
      return (this.customerList.filter(x => x.phoneNumber == phoneNumber.value).length > 0 && this.createOrUpdateForm.value.phoneNumber != this.customerBinding.phoneNumber) ? {
